...
class ISL_Conformer(nn.Module):
    ...
    def forward_single_frame_logits(self, x):
        """Word-level convenience path: x is (B, 1, input_dim). Returns raw (unmasked, non-CTC)
        classification logits (B, num_classes+1) for use with CrossEntropyLoss instead of CTC."""
        lengths = torch.ones(x.size(0), dtype=torch.long, device=x.device)
        log_probs, _ = self.forward(x, lengths)   # (B, 1, num_classes+1)
        return log_probs.squeeze(1)                # (B, num_classes+1), still log-softmax'd
